...
SULUAN is implemented here as a static web prototype for proposal defense and user-flow validation. It demonstrates the approved concept of a UA-governed institutional research intelligence platform without connecting to production systems.

The prototype simulates AI, database, authentication, and backend behavior. Simulated Semantic Search uses predefined temporary scenarios and fallback keyword/theme matching. Role-based access, submitted metadata, and validation changes are stored only in the browser through localStorage.

No Supabase, Firebase, OpenAI API, external database, paid API, backend service, or confidential University of the Assumption record is used. All metadata is fictional sample content created for demonstration.
