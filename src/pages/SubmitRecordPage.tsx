  ...
  return (
    <div>
      ...
      <form onSubmit={submit} className="mt-5 rounded-xl border border-stone-200 bg-white p-5 shadow-sm">
        ...
        <button type="submit" className="mt-5 inline-flex items-center gap-2 rounded-lg bg-[#0038A8] px-4 py-2.5 text-sm font-semibold text-white hover:bg-[#002D72]"><Save className="h-4 w-4" /> Save prototype metadata</button>
        {savedId ? <p className="mt-4 rounded-md bg-[#edf4ff] p-3 text-sm font-semibold text-[#0038A8]">Saved locally as {savedId}. It will appear in Browse and Admin Dashboard on this browser only.</p> : null}
      </form>
    </div>
  )
}
