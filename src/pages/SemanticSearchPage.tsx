    ...
    <div>
      ...
      <section className="mt-5 rounded-lg border border-stone-200 bg-white p-5 shadow-sm">
        <Field label="Research question or topic">
          <div className="flex flex-col gap-3 sm:flex-row">
            ...
            <button type="button" className="inline-flex items-center justify-center gap-2 rounded-md bg-emerald-700 px-4 py-2 text-sm font-semibold text-white">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {semanticScenarios.map((scenario) => (
            <button key={scenario.id} type="button" onClick={() => setQuery(scenario.triggers[0])} className="rounded-md border border-stone-200 px-3 py-1.5 text-xs font-semibold text-stone-700 hover:border-emerald-300 hover:bg-emerald-50">
              {scenario.label}
            </button>
          ))}
        </div>
      ...
      <section className="mt-5">
        <div className="mb-4 rounded-lg border border-stone-200 bg-white p-4 shadow-sm">
          <div className="flex items-center gap-2 text-sm font-bold text-emerald-800"><Brain className="h-4 w-4" /> {simulation.title}</div>
        ...
        <div className="space-y-4">
          ...
          {simulation.results.length === 0 ? <p className="rounded-lg border border-stone-200 bg-white p-5 text-sm text-stone-600">No prototype matches found. Try repository, student support, campus service, or disaster preparedness.</p> : null}
        </div>
      </section>
    </div>
  )
