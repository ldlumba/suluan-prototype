      ...
      <section className="mt-5 rounded-xl border border-stone-200 bg-white p-5 shadow-sm">
        <Field label="Research question or topic">
          <div className="flex flex-col gap-3 sm:flex-row">
            ...
            <button type="button" className="inline-flex items-center justify-center gap-2 rounded-lg bg-emerald-700 px-4 py-2 text-sm font-semibold text-white">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {semanticScenarios.map((scenario) => (
            <button key={scenario.id} type="button" onClick={() => setQuery(scenario.triggers[0])} className="rounded-lg border border-stone-200 px-3 py-1.5 text-xs font-semibold text-stone-700 hover:border-emerald-300 hover:bg-emerald-50">
              {scenario.label}
            </button>
          ))}
        </div>
