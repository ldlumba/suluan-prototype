...
export function BrowsePage() {
  const [filters, setFilters] = useState<RecordFilters>({ ...emptyFilters, validationStatus: 'Validated' })
  ...
  return (
    <div>
      <PageHeader eyebrow="Repository" title="Browse Research Records" description="Search and filter validated fictional sample metadata by department, year, theme, and access status." />
      <SimulationNotice>
      ...
      <section className="mt-5 rounded-lg border border-stone-200 bg-white p-5 shadow-sm">
        ...
        <p className="mt-4 text-sm text-stone-600">Showing <strong>{filteredRecords.length}</strong> of {discoverableRecords.length} validated records.</p>
      </section>
