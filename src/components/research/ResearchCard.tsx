...
export function ResearchCard({ record }: { record: ResearchRecord }) {
  return (
    <article className="group rounded-xl border border-stone-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-[#9bb9ee] hover:shadow-md">
      ...
      <h2 className="mt-4 text-lg font-black leading-7 text-stone-950 group-hover:text-[#0038A8]">{record.title}</h2>
      ...
      <div className="mt-5 flex flex-col gap-3 border-t border-stone-100 pt-4 text-sm sm:flex-row sm:items-center sm:justify-between">
        ...
        <Link to={`/records/${record.id}`} className="inline-flex items-center gap-1 font-bold text-[#0038A8] hover:text-[#002D72]">
